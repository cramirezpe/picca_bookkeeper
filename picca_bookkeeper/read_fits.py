...
def make_chain(names: List[int], mean: List[float], cov: np.ndarray) -> MCSamples:
    ...
    samples = MCSamples(
        samples=gaussian_samples, names=names, labels=[
...
class ReadFits:
    ...
    def __init__(
        ...
        # handle blinded(true) / unblined(false), default = blinded
        blinded: bool = True,
    ):
        """
        ...
        Raises:
        ----------
            ValueError: If neither `fit_file` nor a valid bookkeeper + variation
                        combo is provided.
        """
        if bookkeeper is None:
            if fit_file is None:
                raise ValueError(
                    "Either bookkeeper or fit_file should be provided")
    ...
    def read_fit(self) -> None:
        ...
        with fitsio.FITS(self.fit_file) as hdul:
            ...
            self.pvalue = 1 - \
                sp.stats.chi2.cdf(self.chi2, self.ndata - self.nparams)
        ...
        if ("ap" not in self.names or "at" not in self.names) and (
            ...
            self.values["ap"] = self.values["alpha"] / \
                np.sqrt(self.values["phi"])
            self.values["at"] = self.values["alpha"] * \
                np.sqrt(self.values["phi"])
    ...
    def table_from_fit_data(
        ...
        Parameters can be shown blinded (differences from a baseline apv/vat)
        or unblinded (absolute values). The table includes parameter values
        ± errors, chi^2 per DoF, and p-values.

        Arguments:
        ----------
            fits: List of `ReadFits` instances to include in the table.
            params: List of parameter keys to include
                    (default: ap, at, bias_LYA, beta_LYA).
            params_names: Optional list of display names
                    (defaults to same as `params`).
...
class FitPlots:
    """
    Collection of static methods for visualizing fit results from Vega FITS o
    utput files managed by picca_bookkeeper.

    Core Functionality:
    -------------------
        - Provides visualization tools for correlation functions, cross-correlation
          models, and their 2D map representations.
        - Includes methods for projected correlation functions, error bar plots
          for fit parameters, p-value comparisons,
          and triangle plots for parameter covariances.
        - Supports both blinded and unblinded analysis display modes, including
          comparative plotting across multiple fits.

    Visualization Methods:
    ----------------------
        - cf_model: Plot correlation function model and data for specific
                    regions and absorbers.
        - xcf_model: Visualize cross-correlation function models.
        - cf_model_map, xcf_model_map: Show 2D maps of modelled correlation and
                    cross-correlation functions.
        - rp_model: Plot line-of-sight correlation models averaged over
                    transverse separation.
        - plot_errorbars_from_fit: Plot error bars for selected parameters
                    across fits, supporting blinded display.
        - plot_p_value_from_fit: Show horizontal bar plots of p-values for
                    fit comparisons.
        - plot_triangle: Generate GetDist triangle plots for marginalized
                    parameter distributions.

    Usage:
    ------
        - Visualization functions can be called directly from the FitPlots class,
          either with explicit file paths or by passing a Bookkeeper instance
          for automated path resolution.
       -  Error bar and triangle plotting methods accept lists of ReadFits
          objects to enable multi-fit comparisons.
    ...
    --------
        ...
        rf.compute_chain()
        FitPlots.cf_model(bookkeeper=bk)
        FitPlots.plot_errorbars_from_fit([rf], param="ap")
        FitPlots.plot_triangle(["ap", "at"], readfits=[rf])

    Notes:
    ------
        - All plotting methods accept customization arguments and options to
          save figures or data.
        - Blinding options are supported for analyses requiring baseline
          subtraction.
    """
    @staticmethod
    def cf_model(
        ...
        Arguments:
        ----------
            bookkeeper (Bookkeeper, optional): Object for managing file paths
                and metadata. If not provided, both `fit_file` and
                `correlation_file` must be set manually.
            ...
            r_factor (int): Exponent applied to the radial coordinate for scaling
                (default: 2).
            plot_kwargs (dict): Additional keyword arguments passed to `ax.plot()`
                for customization.
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
        ...
        if save_data and output_prefix is not None:
            np.savez(
                output_prefix.parent /
                    (output_prefix.name + "-plot_cf_model.npz"),
                **{**save_dict, **data_dict},
            )
    ...
    def xcf_model(
        ...
        """
        Plot the cross-correlation function (XCF) model from a Vega fit and
        correlation dataset, with optional data saving or value return.
        ...
        ----------
            ...
            mumin (float): Minimum μ value for the angular wedge (default: 0).
            mumax (float): Maximum μ value for the angular wedge (default: 1).
            abs_mu (bool): If True, use absolute μ values in wedge integration
                (default: True).
            ax (matplotlib.axes.Axes, optional): Matplotlib axis object to plot on.
                A new axis is created if None.
            r_factor (int): Exponent applied to the radial coordinate for scaling
            ...
            plot_kwargs (dict): Additional keyword arguments passed to `ax.plot()`
                ...
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
                                   "aspect": "auto"}
            just_return_values (bool): If True, skip plotting and return the
                radial and model ξ(r) values only.
            output_prefix (Path or str, optional): If set, used as the prefix
                path for saving the output plot data (e.g., `.npz` file).
            save_data (bool): If True, save the plotted data to disk using
                `output_prefix`.
            save_dict (dict): Extra metadata to include in the saved `.npz`
                output file (default: empty dict).
        ...
        if model.size != co.shape[0]:
            ...
            if model.size % N_t != 0:
                ...
                raise ValueError(
                    "Unable to set NP for model. Due to odd number of NP.")

            model_np = model.size // N_t
            remove_idx = (model_np - N_p) // 2
            # this reshapes into data size
            model = model.reshape(model_np, N_t)[
        ...
        if output_prefix is not None:
            if save_data:
                np.savez(
                    output_prefix.parent /
                        (output_prefix.name + "-plot_xcf_model.npz"),
                    **{**save_dict, **data_dict},
                )
        elif save_data:
            raise ValueError("Set output_prefix in order to save data.")
    ...
    def cf_model_map(
        ...
        """
        Plot the 2D correlation function model map from a Vega fit and
        correlation data, optionally saving the figure or returning raw
        matrix values.

        Arguments:
        ----------
            bookkeeper (Bookkeeper, optional): Object for managing file
                paths and metadata.If not provided, both `fit_file` and
                `correlation_file` must be set manually.
            fit_file (Path or str): Path to the Vega fit output file.
                Required if no bookkeeper is used.
            ...
            fig (matplotlib.figure.Figure, optional): Figure object for plotting.
                Required if `ax` is set.
            plot_kwargs (dict): Additional keyword arguments passed to `imshow()`
                for customization.
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
                                   "aspect": "auto"}
            just_return_values (bool): If True, skip plotting and return
                extent and scaled model matrix only.
            output_prefix (Path or str, optional): If set, used as the prefix
                path for saving output files (e.g., `.png`, `.npz`).
            save_data (bool): If True, save the image matrix to a `.npz` file.
            save_plot (bool): If True, save the rendered plot as an image file.
            save_dict (dict): Additional metadata to include in the saved `.npz`
                file (default: empty dict).
    ...
    def rp_model(
        ...
        """
        Plot the line-of-sight (rp) correlation model (auto or cross),
        averaged over a specified transverse (rt) range, from a Vega fit output
        and correlation data.

        Arguments:
        ----------
            bookkeeper (Bookkeeper, optional): Object to manage file
                paths and metadata. If not provided, both `fit_file` and
                `correlation_file` must be given.
            ...
            correlation_file (Path or str): Path to the 2D correlation `.fits` file.
                Required if no bookkeeper is used.
            auto (bool, optional): Whether to use the auto-correlation model.
                If False, cross-correlation with quasars is assumed (default: False).
            region (str): Region name of the first tracer (default: "lya").
            region2 (str, optional): Region name of the second tracer.
                Defaults to `region`.
            absorber (str): Absorber name for the first tracer (default: "lya").
            absorber2 (str, optional): Absorber name for the second tracer.
                Defaults to `absorber`.
            rtmin (float): Minimum transverse separation (rt) to include in
                average (default: 0).
            rtmax (float): Maximum transverse separation (rt) to include in
                average (default: 1).
            ax (matplotlib.axes.Axes, optional): Axis object to plot on.
                If None, a new figure is created.
            r_factor (int): Not currently used in this function but reserved
                for compatibility (default: 2).
            plot_kwargs (dict): Additional keyword arguments passed to
                `imshow()` for customization.
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
                                   "aspect": "auto"}
            just_return_values (bool): If True, skip plotting and return the
                rp/data arrays directly.
            output_prefix (Path or str, optional): If set, prefix path to save
                output `.npz` data file.
            save_data (bool): If True, save the computed rp and data arrays
                to a `.npz` file.
        ...
        ----------
            Tuple[np.ndarray, np.ndarray]:
                ...
                - `data`: Averaged correlation function values over the given
                          rt wedge.
        """
        if output_prefix is not None:
            output_prefix = Path(output_prefix)
        ...
        if save_data and output_prefix is not None:
            np.savez(
                output_prefix.parent /
                    (output_prefix.name + f"-plot_{name}_model.npz"),
                **{**save_dict, **data_dict},
            )
    ...
    def xcf_model_map(
        ...
        """
        Plot a 2D cross-correlation model map as a function of line-of-sight
        (rp) and transverse (rt) separations, weighted by a radial factor.

        This function retrieves and reshapes model output from a Vega
        `.fit.pkl` file, aligns it to the dimensions of a correlation function
        `.fits` file, and plots a colormap of the result.

        Arguments:
        ----------
            bookkeeper (Bookkeeper, optional): Object to manage file paths and
                metadata. If not provided, both `fit_file` and `correlation_file`
                must be given.
            fit_file (Path or str): Path to the Vega model `.fit.pkl` file.
                Required if no bookkeeper is used.
            correlation_file (Path or str): Path to the 2D correlation `.fits` file.
                Required if no bookkeeper is used.
            region (str): Region name of the Lyman-alpha forest or tracer
                (default: "lya").
            absorber (str): Absorber name to cross-correlate with quasars
                (default: "lya").
            ax (matplotlib.axes.Axes, optional): Axis object to plot on.
                If None, one is created.
            r_factor (int): Exponential factor applied to the radial distance
                weighting in the colormap (default: 2).
            vmin (float): Minimum value for the colormap scale (default: -0.4).
            vmax (float): Maximum value for the colormap scale (default: 0.4).
            fig (matplotlib.figure.Figure, optional): Required if `ax` is given;
                figure where the axis belongs.
            plot_kwargs (dict): Additional keyword arguments passed to `imshow()`
                for customization.
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
                                   "aspect": "auto"}
            just_return_values (bool): If True, skip plotting and return extent
                and data matrix directly.
            output_prefix (Path or str, optional): If set, prefix path for
                saving output plot and/or data.
            save_data (bool): Whether to save the computed matrix to a `.npz` file.
            save_plot (bool): Whether to save the plot as a `.png` image.
            save_dict (dict): Extra metadata to include in the saved `.npz`
                file if `save_data` is True.
    ...
    def plot_errorbars_from_fit(
        ...
        In blinded mode, it shifts values like `ap` and `at` relative to a
        reference and suppresses their absolute scale.

        In unblinded mode, it shows true parameter values and highlights
        reference bands.
        ...
        ----------
            ...
            plot_kwargs (dict): Additional keyword arguments passed to
                `ax.errorbar()` for customization.
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
                                   "aspect": "auto"}
            reference (ReadFits, optional): Reference `ReadFits` object used
                to define shaded bands and baseline value.
            blinded (bool): If True, hides the absolute parameter values
                (e.g., shows delta_ap = ap - baseline) and centers the reference
                at 0 (default: True).

        Returns:
        ----------
            List[matplotlib.container.Container | Any]:
                List of matplotlib handles for plotted objects
                (e.g., error bars, lines).
        """
    ...
    def plot_p_value_from_fit(
        ...
        """
        Plot horizontal bars showing p-values for a set of fits, optionally
            ...
            `ax.errorbar()` for customization.
                For example:
                    plot_kwargs = {"ls": '--',
                                   "color": 'black',
                                   "aspect": "auto"}
            reference (ReadFits, optional): Reference fit whose p-value is
        ...
        ----------
            List[matplotlib.container.Container | Any]:
                List of matplotlib handles for plotted objects
                (e.g., error bars, lines), useful for legend construction.
        """
        if ax is None:
            fig, ax = plt.subplots()
    ...
    def plot_triangle(
        ...
        """
        Plot a GetDist triangle plot for MCMC chains associated with ReadFits
        or provided directly.

        This generates 1D and 2D marginalized posterior distributions for the
        specified parameters using `getdist`. You can pass either a list of
        `ReadFits` objects with attached chains or the `chains` themselves directly.

        Arguments:
        ----------
            param_names (List[str]): List of parameter names to plot
                (must match those in the chains).
            readfits (List[ReadFits], optional): List of fits with MCMC chains to plot.
                If provided, chains will be extracted from `fit.chain`.
            chains (List[getdist.MCSamples], optional): List of `MCSamples`
                chains to plot directly.
            labels (List[str | None], optional): Labels for each chain.
                If `None`, uses `fit.label`.
            colours (List[str | None], optional): Contour colors for each chain.
                If `None`, uses `fit.colour`.
            g (getdist.plots.GetDistPlotter, optional): A `GetDistPlotter` instance.
        ...
        Note:
        ----------
            Automatically sets labels and colors from the `ReadFits` metadata
